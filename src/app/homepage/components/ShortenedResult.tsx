...
const ShortenedResult = ({ originalUrl, shortCode, shortUrl, qrCodeDataUrl, onClose }: ShortenedResultProps) => {
  ...
  const handleCopyQr = async () => {
    if (!isHydrated || !navigator?.clipboard) return;
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_180px] items-start">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3 flex flex-col items-center gap-3">
          <img src={qrCodeDataUrl} alt={`QR code for ${shortUrl}`} className="w-full max-w-[140px] rounded-xl border border-white/8" />
          <button
            onClick={handleCopyQr}
            className="w-full px-4 py-2.5 rounded-xl text-sm font-semibold text-white/80 bg-white/[0.05] border border-white/10 transition-all duration-250 hover:bg-white/[0.08] flex items-center justify-center gap-2"
          >
            <Icon name={qrCopied ? 'CheckIcon' : 'QrCodeIcon'} size={16} variant="outline" />
      ...
      <div className="mt-5 pt-5 border-t border-white/8 flex flex-col sm:flex-row gap-3">
        {loading ? (
          <div
            className="flex-1 text-center px-5 py-3 font-body font-semibold text-sm text-white/70 rounded-xl"
          ...
          <Link
            href={`/link-analytics?code=${shortCode}`}
